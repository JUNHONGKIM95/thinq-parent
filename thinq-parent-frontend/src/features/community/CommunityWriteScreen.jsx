...
const BOARD_OPTIONS = ['임신 수다', '고민 / 정보']
const PREFIX_OPTIONS = ['증상기록', '아기상태', '진료정보', '건강관리', '출산준비', '사회지원', '감정기록']
        ...
        <div className="community-write-header-actions">
              ...
              <div className="community-keyword-menu community-write-dropdown" role="listbox" aria-label="게시판 선택">
                {BOARD_OPTIONS.map((item) => (
                  <button
                    ...
                    type="button"
                    role="option"
                    aria-selected={selectedBoard === item}
                    className={`community-keyword-item ${selectedBoard === item ? 'is-active' : ''}`}
                    onClick={() => selectOption('board', item)}
                  >
                    {selectedBoard === item ? (
          ...
          <div className="community-write-select">
            <button
              type="button"
              className="community-search-trigger community-write-select-trigger"
              onClick={() => setOpenMenu((current) => (current === 'prefix' ? null : 'prefix'))}
              aria-expanded={openMenu === 'prefix'}
              aria-haspopup="listbox"
            >
              <span>{selectedPrefix || '머리말'}</span>
              <DownIcon />
            </button>

            {openMenu === 'prefix' ? (
              <div className="community-keyword-menu community-write-dropdown" role="listbox" aria-label="머리말 선택">
                {PREFIX_OPTIONS.map((item) => (
                  <button
                    key={item}
                    type="button"
                    role="option"
                    aria-selected={selectedPrefix === item}
                    className={`community-keyword-item ${selectedPrefix === item ? 'is-active' : ''}`}
                    onClick={() => selectOption('prefix', item)}
                  >
                    {selectedPrefix === item ? (
                      <img src={keywordActiveBackground} alt="" className="community-keyword-item-bg" aria-hidden="true" />
                    ) : null}
          ...
          <button type="button" className="community-write-submit-button">
            글쓰기
          </button>
        </div>
      </header>
